...
async def upload_flipbook(file: UploadFile = File(...), user=Depends(require_roles("pembina", "waka"))):
    ...
    if len(content) > 10 * 1024 * 1024:
        raise HTTPException(413, "File terlalu besar, maksimal 10MB")
...
async def create_magazine(payload: MagazineInput, user=Depends(require_roles("pembina", "waka"))):
    if not payload.pdf_url:
        raise HTTPException(400, "pdf_url wajib diisi")
...
async def update_magazine(magazine_id: str, payload: MagazineInput, user=Depends(require_roles("pembina", "waka"))):
    existing = await db.magazines.find_one({"id": magazine_id}, {"_id": 0})
    if not existing:
        raise HTTPException(404, "Majalah tidak ditemukan")
    update_data = {"title": payload.title, "description": payload.description}
    if payload.pdf_url:
        update_data["pdf_url"] = payload.pdf_url
...
async def get_magazine_pdf(magazine_id: str):
    ...
    if mag.get("pdf_url"):
        ...
        return RedirectResponse(url=mag["pdf_url"])
